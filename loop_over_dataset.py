...
#results_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'results/' + 'fpn-' + model + '/results_sequence_' + sequence + '_' + model) #step3
datafile = WaymoDataFileReader(data_fullpath)
datafile_iter = iter(datafile)  # initialize dataset iterator

## Initialize object detection
configs_det = det.load_configs(model_name='darknet') # options are 'darknet', 'fpn_resnet'
#configs_det = det.load_configs(model_name='fpn_resnet') # options are 'darknet', 'fpn_resnet' #step3
model_det = det.create_model(configs_det)

configs_det.use_labels_as_objects = False # True = use groundtruth labels as objects, False = use model-based detection
#configs_det.use_labels_as_objects = True # step 4 final
