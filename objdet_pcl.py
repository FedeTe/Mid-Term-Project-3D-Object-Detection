...
def show_range_image(frame, lidar_name):
    ...
    cv2.imshow('range image + intensity', cv2.resize(img_range_intensity,(1500,120)))
    cv2.waitKey(0)
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_cpy[lidar_pcl_cpy[:, 1] < 0] = 0

    # step 4 : visualize point-cloud using the function show_pcl from a previous task
    #lidar_pcl_cpy = lidar_pcl_cpy.astype(np.uint8)
    ...
    #lidar_pcl_cpy[lidar_pcl_cpy[:, 3] < 0.001, 3] = 0.0001
    ...
    #lidar_top_pcl[lidar_top_pcl[:, 3] > 0.1, 3] = 0.1
    ...
    while(1):
        cv2.imshow('bev_intensity', img_intensity)
    ...
    while(1):
        cv2.imshow('bev_height', img_height)
        if cv2.waitKey(10) & 0xFF == 32:
            break
    cv2.destroyAllWindows()
